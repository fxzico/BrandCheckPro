...
function brandCheckApp() {
  return {
    ...
    init() {
      ...
      // Live system-appearance listener — matches OS preference only if no manual local override
      const mq = window.matchMedia('(prefers-color-scheme: dark)');
      const applyTheme = (e) => {
        if (localStorage.getItem('theme')) {
          // If there is an explicit theme setting, respect it rather than changing it based on OS scheme changes
          const activeTheme = localStorage.getItem('theme');
          this.darkMode = (activeTheme === 'dark');
          if (this.darkMode) {
